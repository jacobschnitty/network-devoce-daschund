...
mac_queue = Queue()
websocket_queue = Queue()  # Queue for WebSocket
    ...
    try:
        async with websockets.connect(WS_URI) as websocket:
            ...
            while True:
                ...
                console.print(f"[cyan]Data sent to WebSocket:[/]{json.dumps(message)}")
                console.log(f"Putting data in WebSocket queue - MAC: {mac_address}, IP: {ip_address}, Hostname: {hostname}")
...
async def websocket_loopback_listener():
    '''WebSocket listener for receiving loopback data from Node-RED'''
    WS_LOOPBACK_URI = "192.168.1.252:1880/ws/loopback"
    try:
        async with websockets.connect(WS_LOOPBACK_URI) as websocket:
            console.print(f"[green]Connected to WebSocket loopback server at {WS_LOOPBACK_URI}[/green]")
            while True:
                # Receive the loopback message
                message = await websocket.recv()
                data = json.loads(message)
                
                # Log loopback data to the console for checking
                console.log(f"[blue]Loopback data received:[/] {data}")

                # Add the data to the loopback queue for further processing if necessary
                loopback_queue.put(data)
    except websockets.ConnectionClosedError as e:
        console.print(f"[red]Loopback WebSocket connection closed: {e}[/red]")
    except Exception as e:
        console.print(f"[red]Error in WebSocket loopback communication: {e}[/red]")
    finally:
        console.print(f"[yellow]WebSocket loopback connection has been closed.[/yellow]")
...
def handle_packet(packet):
    ...
    try:
        if hasattr(packet, 'eth') and hasattr(packet, 'ip'):
            ...
            console.log(f"[yellow]Packet does not have Ethernet or IP layers[/yellow]")
