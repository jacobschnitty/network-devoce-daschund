...
import readchar  # For capturing arrow keys
...
NODE_RED_SERVER_IP = 'your-node-red-server-ip'  # Replace with the actual IP
NODE_RED_SERVER_PORT = 12345                    # Replace with the actual port number
...
def select_interface_with_arrows(interfaces):
    '''Allows user to select network interface using arrow keys'''
    selected_index = 0
    while True:
        # Clear console and display the interfaces
        console.clear()
        ...
        for i, iface in interfaces.items():
            if i == selected_index:
                ...
                table.add_row(f"[bold yellow]{i}[/bold yellow]", f"[bold yellow]{iface['name']}[/bold yellow]", f"[bold yellow]{iface['guid']}[/bold yellow]")
            else:
        ...
        if key == readchar.key.UP:
            ...
            return selected_index  # Return the selected index when Enter is pressed
...
def main():
    ...
    if not interfaces:
        console.print("[bold red]No interfaces found.[/bold red]")
        return

    # Allow user to select interface with arrow keys
    selected_index = select_interface_with_arrows(interfaces)
