...
class MACAddressCounter(Thread):
    ...
    def send_data_to_node_red(self, mac_address, ip_address, hostname, status):
        '''Send data to Node-RED via TCP and provide feedback to the terminal'''
        try:
            # Get the Node-RED server details (IP and port) from user or defaults
            NODE_RED_SERVER_IP, NODE_RED_SERVER_PORT = get_node_red_server_details()

            # Attempt to establish a connection to Node-RED server
            console.print(f"[yellow]Attempting connection to {NODE_RED_SERVER_IP}:{NODE_RED_SERVER_PORT}...[/yellow]")
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                sock.settimeout(10)  # Set a timeout to avoid hanging on connection attempts
                sock.connect((NODE_RED_SERVER_IP, NODE_RED_SERVER_PORT))
                console.print(f"[green]Connection established with {NODE_RED_SERVER_IP}:{NODE_RED_SERVER_PORT}[/green]")
                ...
                # Sending the message to Node-RED
                sock.sendall(message_json.encode('utf-8'))
                console.print(f"[cyan]Data sent to Node-RED:[/] {message_json}")
        
        except ConnectionRefusedError:
            console.print(f"[red]Error: Connection to {NODE_RED_SERVER_IP}:{NODE_RED_SERVER_PORT} refused. Is Node-RED running?[/red]")
        
        except socket.timeout:
            console.print(f"[red]Error: Connection to {NODE_RED_SERVER_IP}:{NODE_RED_SERVER_PORT} timed out.[/red]")
...
def get_node_red_server_details():
    ...
    default_ip = 'your-node-red-server-ip'  # Replace with the default IP
    default_port = 12345  # Replace with the default port
